package Q3;

/**
 * A class to represent a binary search tree.
 */
public class BinarySearchTree<E extends Comparable<E>>
        extends AgeData{
    // Data Fields

    AgeData root;
    /** Return value from the public add method. */
    protected boolean addReturn;
    /** Return value from the public delete method. */
    protected AgeData deleteReturn;

    //Methods
    /**
     * Starter method find.
     * @param target The Comparable object being sought
     * @return The object, if found, otherwise null
     */
    // @Override
    public AgeData find(AgeData target) {
        return find(root, target);
    }

    /**
     * Recursive find method.
     * @param localRoot The local subtree�s root
     * @param target The object being sought
     * @return The object, if found, otherwise null
     */
    private AgeData find(AgeData localRoot, AgeData target) {
        if (localRoot == null) {
            return null;
        }

        // Compare the target with the data field at the root.
        int compResult = target.compareTo(localRoot);
        if (compResult == 0) {
            return localRoot;
        } else if (compResult < 0) {
            return find(localRoot.left, target);
        } else {
            return find(localRoot.right, target);
        }
    }

    /**
     * Starter method add.
     * @param item The object being inserted
     * @return true if the object is inserted, false
     *         if the object already exists in the tree
     */
    // @Override
    public boolean add(AgeData item) {
        root = add(root, item);
        return addReturn;
    }

    /**
     * Recursive add method.
     * @param localRoot The local root of the subtree
     * @param item The object to be inserted
     * @return The new local root that now contains the
     *         inserted item
     */
    private AgeData add(AgeData localRoot, AgeData item) {
        if (localRoot == null) {
            // item is not in the tree � insert it.
            addReturn = true;
            return new AgeData(item);
        } else if (item.compareTo(localRoot) == 0) {
            // item is equal to localRoot.data
            addReturn = false;
            localRoot.person++;
            return localRoot;
        } else if (item.compareTo(localRoot) < 0) {
            // item is less than localRoot.data
            localRoot.left = add(localRoot.left, item);
            return localRoot;
        } else {
            // item is greater than localRoot.data
            localRoot.right = add(localRoot.right, item);
            return localRoot;
        }
    }

    /**
     * Starter method delete.
     * @param target The object to be deleted
     * @return The object deleted from the tree
     *         or null if the object was not in the tree
     * @throws ClassCastException if target does not implement
     *         Comparable
     */
    public AgeData delete(AgeData target) {
        root = delete(root, target);
        return deleteReturn;
    }

    /**
     * Recursive delete method.
     *       deleteReturn is equal to the deleted item
     *       as it was stored in the tree or null
     *       if the item was not found.
     * @param localRoot The root of the current subtree
     * @param item The item to be deleted
     * @return The modified local root that does not contain
     *         the item
     */
    private AgeData delete(AgeData localRoot, AgeData item) {
        if (localRoot == null) {
            // item is not in the tree.
            deleteReturn = null;
            return localRoot;
        }

        // Search for item to delete.
        int compResult = item.compareTo(localRoot);
        if (compResult < 0) {
            // item is smaller than localRoot.data.
            localRoot.left = delete(localRoot.left, item);
            return localRoot;
        } else if (compResult > 0) {
            // item is larger than localRoot.data.
            localRoot.right = delete(localRoot.right, item);
            return localRoot;
        } else {
            if(localRoot.person > 1 ){
                localRoot.person--;
                return localRoot;
            }
            else{
                // item is at local root.
                deleteReturn = localRoot;
                if (localRoot.left == null) {
                    // If there is no left child, return right child
                    // which can also be null.
                    return localRoot.right;
                } else if (localRoot.right == null) {
                    // If there is no right child, return left child.
                    return localRoot.left;
                } else {
                    // Node being deleted has 2 children, replace the data
                    // with inorder predecessor.
                    if (localRoot.left.right == null) {
                        // The left child has no right child.
                        // Replace the data with the data in the
                        // left child.
                        localRoot.data = localRoot.left.data;
                        // Replace the left child with its left child.
                        localRoot.left = localRoot.left.left;
                        return localRoot;
                    } else {
                        // Search for the inorder predecessor (ip) and
                        // replace deleted node's data with ip.
                        localRoot = findLargestChild(localRoot.left);
                        return localRoot;
                    }
                }
            }

        }
    }

    /**
     * Find the node that is the
     * inorder predecessor and replace it
     * with its left child (if any).
     * @post The inorder predecessor is removed from the tree.
     * @param parent The parent of possible inorder
     *        predecessor (ip)
     * @return The data in the ip
     */
    private AgeData findLargestChild(AgeData parent) {
        // If the right child has no right child, it is
        // the inorder predecessor.
        if (parent.right.right == null) {
            AgeData returnValue = parent.right;
            parent.right = parent.right.left;
            return returnValue;
        } else {
            return findLargestChild(parent.right);
        }
    }

    /**
     * write all node to string
     * @return string
     */
    public String toString() {
        StringBuilder sb = new StringBuilder();
        this.writeTree(root, sb);
        return sb.toString();
    }

    /**
     * write all node to string
     * @param node node for iterator
     * @param sb string
     */
    private void writeTree(AgeData node, StringBuilder sb) {

        if (node == null) {
            sb.append("null\n");
        } else {
            sb.append(node.toString());
            sb.append("\n");
            this.writeTree(node.left, sb);
            this.writeTree(node.right, sb);
        }

    }

    /**
     *  returns the number of people younger than an age
     * @param age age
     * @return how many people
     */
    public int youngerThan(int age) {
        return findYounger(root, age);
    }

    /**
     * @param node
     * @param age
     * @return people in the same age + next node
     */
    private int findYounger(AgeData node, int age) {
        if (node == null)
            return 0;
        else if(node.data < age){
            return findYounger(node.left, age) + node.person + findYounger(node.right, age);
        }
        else if(node.left != null)
            return findYounger(node.left, age);

        return 0;
    }

    /**
     *  returns the number of people older than an age
     * @param age age
     * @return how many people
     */
    public int olderThan(int age) {
        return olderThan(root,  age);
    }

    /**
     * @param node
     * @param age
     * @return people in the same age + next node
     */
    private int olderThan(AgeData node, int age) {
        if (node == null)
            return 0;
        else if(node.data > age){
            return olderThan(node.right, age) + node.person + olderThan(node.left, age);
        }
        else if(node.right != null)
            return olderThan(node.right, age);

        return 0;
    }

}


